...
def get_user_genres(user_rated_movies): #list of genres collected based on user list
    user_genres = []
...
def genre_check(genre_ids, user_genres): #checks of genres of movies recommended align with user interest
    movie_genres = eval(genre_ids)
    return any(int(genre) in user_genres for genre in movie_genres)

def get_genres(genre_ids): #checks for genre id and its matching genre type for output
    genres = eval(genre_ids)
...
if __name__ == "__main__":
    #edit for hadoop 
    movies_df = pd.read_csv('master_movies.csv')
    user_ratings_df = pd.read_csv('users_list.csv')

    user_input_id = int(input("Enter the User ID for a list of 10 recommendations based on previous movie ratings: "))
